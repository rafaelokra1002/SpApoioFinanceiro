...
import {
  ...
  Video, Camera, Package, FileText, Check, ChevronDown,
} from 'lucide-react';
...
const MEDIA = {
  video: 'Eletrônico — Vídeo',
  foto1: 'Eletrônico — Foto 1 (item)',
  foto2: 'Eletrônico — Foto 2 (caixa)',
  foto3: 'Eletrônico — Foto 3 (nota fiscal)',
} as const;
...
const MIDIAS: { key: MediaKey; titulo: string; sub: string; video?: boolean; icon: React.ReactNode }[] = [
  { key: 'video', titulo: 'Vídeo do item', sub: 'Máx. 60s', video: true, icon: <Video size={18} color="#2546f0" strokeWidth={2} /> },
  { key: 'foto1', titulo: 'Foto 1', sub: 'do item', icon: <Camera size={18} color="#2546f0" strokeWidth={2} /> },
  { key: 'foto2', titulo: 'Foto 2', sub: 'da caixa', icon: <Package size={18} color="#2546f0" strokeWidth={2} /> },
  { key: 'foto3', titulo: 'Foto 3', sub: 'da nota fiscal', icon: <FileText size={18} color="#2546f0" strokeWidth={2} /> },
];

export function Eletronico() {
  ...
  const setG = (patch: Partial<typeof g>) =>
    dispatch({ type: 'SET_FIELD', field: 'garantiaEletronico', value: { ...g, ...patch } });
      ...
      <div style={{ padding: '18px 20px 6px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: '#0d2b5e', letterSpacing: '-0.02em', lineHeight: 1.12 }}>
              Eletrônicos e Celulares
            </h1>
            <p style={{ marginTop: 6, fontSize: 13.5, lineHeight: 1.4, color: '#6b7280' }}>
              Informe os detalhes do eletrônico que será oferecido como garantia.
            </p>
          </div>
          <EletronicoIlustra />
        </div>
      ...
      <div style={{ background: '#fff', position: 'relative', padding: '16px 18px 22px' }}>
        ...
        <Campo label="Modelo">
          <input style={inputStyle} value={g.modelo}
            placeholder="Ex: Galaxy S23 / Inspiron 15 / Smart TV"
            onChange={e => setG({ modelo: e.target.value })}
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 11 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <label style={miniLabel}>Capacidade / Especificação</label>
            <input style={inputStyle} value={g.capacidade}
              placeholder={'Ex: 256GB / 16GB RAM / 55"'}
              onChange={e => setG({ capacidade: e.target.value })}
              onFocus={foco} onBlur={desfoco} />
          </div>
        </div>

        {/* Acessórios */}
        <h2 style={{ ...secTitle, marginTop: 24 }}>Acessórios e itens inclusos</h2>
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ flex: 1 }}>
            <label style={miniLabel}>Caixa</label>
            <SimNao value={g.temCaixa} onChange={val => setG({ temCaixa: val })} />
          </div>
          <div style={{ flex: 1 }}>
            <label style={miniLabel}>Nota fiscal</label>
            <SimNao value={g.temNotaFiscal} onChange={val => setG({ temNotaFiscal: val })} />
          </div>
          <div style={{ flex: 1 }}>
            <label style={miniLabel}>Carregador</label>
            <SimNao value={g.temCarregador} onChange={val => setG({ temCarregador: val })} />
          </div>
        </div>

        <div style={{ marginTop: 14 }}>
          <label style={miniLabel}>Valor de mercado (R$)</label>
          <input style={{ ...inputStyle, maxWidth: 220 }} value={g.valorMercado} inputMode="numeric"
        ...
        {/* Mídias */}
        <h2 style={{ ...secTitle, marginTop: 24 }}>Mídias do item</h2>
        <p style={{ fontSize: 12, color: '#6b7280', marginTop: -8, marginBottom: 10 }}>
          Envie 1 vídeo e 3 fotos nítidas do item, da caixa e da nota fiscal.
        </p>
              ...
              <button key={m.key} onClick={() => abrirSeletor(m.key, !!m.video)}
                ...
                <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                  <div style={{
                    ...
                    {file ? <Check size={18} color="#fff" strokeWidth={3} /> : m.icon}
                  </div>
      ...
      {options.map(op => {
        ...
        return (
          <button key={op.value} onClick={() => onChange(op.value)}
            style={{
              flex: 1, padding: '9px 8px', borderRadius: 9, cursor: 'pointer', border: 'none',
              background: active ? '#123bd6' : 'transparent',
              color: active ? '#fff' : '#6b7280', fontWeight: 700, fontSize: 13,
...
function EletronicoIlustra() {
  return (
    <img
      src="/eletronico-hero.png"
      alt="Eletrônicos oferecidos como garantia"
      style={{
        flexShrink: 0, width: 148, height: 'auto', display: 'block',
